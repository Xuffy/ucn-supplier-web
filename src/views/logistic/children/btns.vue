  ...
  <el-row class="btns" :style="{ width: '100%', paddingLeft: hideMune ? '65px' : '195px' }">
    <div v-if="!edit">
      ...
      <div v-else>
        ...
        <el-button size="mini" type="primary" v-if="pageType=='loadingListDetail'&&(logisticsStatus.status==1||logisticsStatus.status==2||logisticsStatus.status==3)"
          @click.stop="$emit('switchEdit', 'modify')">{{ $i.logistic.modify }}</el-button>
        ...
        <el-button size="mini" type="primary" v-if="logisticsStatus.status==2&&DeliveredEdit" @click.stop="$emit('sendData', 'send')">{{ $i.logistic.confirmModify }}</el-button>
