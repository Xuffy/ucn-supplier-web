'use strict';
export default {
    //order
    get_orderlist: ['/order/overview/page', 'BASE_HCJ', 'USUPPLY_URL'],
    get_draft_orderlist: ['/order/draft/page', 'BASE_HCJ', 'USUPPLY_URL'],
    get_recycle_orderlist: ['/order/recycle/page', 'BASE_HCJ', 'USUPPLY_URL'],
    post_recover_order: ['/order/recover', 'BASE_HCJ', 'USUPPLY_URL'],
    post_finishPost: ['/order/finish', 'BASE_HCJ', 'USUPPLY_URL'],
    delete_order: ['/order/delete', 'BASE_HCJ', 'USUPPLY_URL'],
    download_order: ['/order/download', 'BASE_HCJ', 'USUPPLY_URL'],
    send_order: ['/order/send', 'BASE_HCJ', 'USUPPLY_URL'],
    add_order: ['/order/save', 'BASE_HCJ', 'USUPPLY_URL'],
    detail_order: ['/order/detail/{id}', 'BASE_HCJ', 'USUPPLY_URL'],
    get_order_history: ['/order/skuHistory', 'BASE_HCJ', 'USUPPLY_URL'],
    post_order_paymentlist: ['/payment/list/{orderNo}/{orderType}', 'BASE_BUYER', 'USUPPLY_URL'],
    post_order_skus: ['/order/skus', 'BASE_HCJ', 'USUPPLY_URL'],
    post_cancleOrder: ['/order/cancle', 'BASE_HCJ', 'USUPPLY_URL'],
    post_updataOrder: ['/order/update', 'BASE_HCJ', 'USUPPLY_URL'],
    post_confirm: ['/order/confirm  ', 'BASE_HCJ', 'USUPPLY_URL'],
    //supplier
    post_add_supplier_remark: ['/purchase/supplier/remark', 'USUPPLY_URL'],
    get_update_supplier_remark: ['/purchase/supplier/remark/{id}', 'USUPPLY_URL'],
    get_listSupplier: ['/purchase/listSupplier', 'USUPPLY_URL', 'USUPPLY_URL'],
    get_supplier_id: ['/purchase/supplier/{id}', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_list_remark: ['/purchase/supplier/listRemarks', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_remark: ['/purchase/supplier/remark', 'USUPPLY_URL', 'USUPPLY_URL'],
    get_supplier_remark_de: ['/purchase/supplier/remark/{id}', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_delete_remark: ['/purchase/supplier/deleteRemark/{id}', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_listSupplierByIds: ['/purchase/supplier/listSupplierByIds', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_addbookmark: ['/purchase/supplier/bookmark', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_deletebookmark: ['/purchase/supplier/deleteBookmark/{id}', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_listbookmark: ['/purchase/supplier/listBookmarks', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_recoverbookmark: ['/purchase/supplier/recoverBookmark/{id}', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_addCompare: ['/purchase/supplier/compare', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_listCompare: ['/purchase/supplier/listCompare', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_listCompareDetails: ['/purchase/supplier/listCompareDetails', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_deleteCompareDetails: ['/purchase/supplier/deleteCompareDetails', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_deleteCompare: ['/purchase/supplier/deleteCompare', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_supplier_recoverCompare: ['/purchase/supplier/recoverCompare', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_deleteBookmarks: ['/purchase/supplier/batchDeleteBookmark', 'USUPPLY_URL', 'USUPPLY_URL'],
    post_recoverBookmarks: ['/purchase/supplier/batchRecoverBookmark', 'USUPPLY_URL', 'USUPPLY_URL'],

    //customer
    post_getCustomerList: ['/supply/overview', 'BASE_CS', 'USUPPLY_URL'],
    post_deleteCustomerRemark: ['/supply/deleteRemark/{id}', 'BASE_CS', 'USUPPLY_URL'],
    post_customerDetail: ['/supply/{id}', 'BASE_CS', 'USUPPLY_URL'],
    post_getCustomerListRemark: ['/supply/listRemarks/{id}', 'BASE_CS', 'USUPPLY_URL'],
    post_addCustomerListRemark: ['/supply/remark', 'BASE_CS', 'USUPPLY_URL'],
    post_delete_CustomerRemark: ['/supply/deleteRemark/{id}', 'BASE_CS', 'USUPPLY_URL'],
    post_customerUpdataRmark: ['/supply/remark/{id}', 'BASE_CS', 'USUPPLY_URL'],


    //payment
    paymentGetNo: ['/payment/genNo', 'BASE_HCJ', 'USUPPLY_URL'],
    paymentSave: ['/payment/save', 'BASE_HCJ', 'USUPPLY_URL'],
    paymentUpdata: ['/payment/update', 'BASE_HCJ', 'USUPPLY_URL'],
    paymentRecover: ['/payment/recover', 'BASE_HCJ', 'USUPPLY_URL'],
    paymentAbandon: ['/payment/abandon', 'BASE_HCJ', 'USUPPLY_URL'],
    paymentAccept: ['/payment/accept/{id}', 'BASE_HCJ', 'USUPPLY_URL'],
    paymentDunning: ['/payment/dunning/{orderNo}/{orderType}', 'BASE_HCJ', 'USUPPLY_URL'],
    post_order_paymentlist: ['/payment/list/{orderNo}/{orderType}', 'BASE_HCJ', 'USUPPLY_URL'],
    
    
    //字典表数据
    post_codePart:['/code/part','UMETA_URL'],
     post_country:['/country/all','UMETA_URL'],
     post_logisticsport:['/logisticsport/query','UMETA_URL'],
     get_currency:['/currency/all','UMETA_URL']
}
